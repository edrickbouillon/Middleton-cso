...
const steps = [
  ...
  { id: 2,  text: "<span class='title'>Spill to storm tank</span>During heavy rain, the sewer can fill up as wastewater mixes with rainwater, causing water levels to rise. Once it reaches this higher pipe, the excess flow is screened to remove debris before being carried to the storage tank." },

  { id: 3,  text: "<span class='title'>Tank inlet</span>This is where stormwater enters the storage tank during heavy rain. Most of the time it stays dry, but when the system is full, excess stormwater flows in here to stop the sewer network from overflowing." },

  { id: 4,  text: "<span class='title'>Underground storage tank</span>This tank stores excess stormwater underground until there is space for it in the sewers again." },
  ...
  { id: 6,  text: "<span class='title'>Taps</span>These act as a one-way system, allowing water to flow out of the storage tank but closing if it tries to flow back the wrong way. This keeps the flow moving safely back to the sewer network after heavy rain." },

  { id: 7,  text: "<span class='title'>Return water</span>Once the heavy rain has passed, this pipe carries stormwater from the emptied storage tank back into the sewer network, where it continues to the treatment works as normal." },
  ...
  { id: 10, text: "<span class='title'>Overflow to environment</span>This overflow is only used when both the storm tank and storage tank are completely full. In this case, any remaining stormwater is screened before being released into the river." },

  { id: 11, text: "<span class='title'>Engineer</span>Our engineers work hard, checking the tank and equipment to make sure everything is working properly." }
];
...
/* Esc to close (returns focus to the burger button) */
...
toggleBtn.onclick = () => {
  ...
  if (backgroundEnabled) {
    viewer.setAttribute("environment-image", "spruit_sunrise_1k_HDR.hdr");
...
const DARK_STORAGE_KEY = "ysa-dark";

function setDarkMode(on) {
  document.body.classList.toggle("dark", on);
  darkToggle.classList.toggle("active",   on);
  darkToggle.classList.toggle("inactive", !on);
  darkToggle.setAttribute("aria-pressed", on ? "true" : "false");
  viewer.setAttribute("background-color", on ? "#0c1228" : "#ffffff");
  try { localStorage.setItem(DARK_STORAGE_KEY, on ? "1" : "0"); } catch (_) {}
}
...
/* Restore stored preference on load */
try {
  if (localStorage.getItem(DARK_STORAGE_KEY) === "1") setDarkMode(true);
} catch (_) {}
